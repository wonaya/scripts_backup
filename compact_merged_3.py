...
outfile = open("merged_all_nolog.txt" ,'w')
...
for gene in gene_list :
    ...
    for a in open(sys.argv[1],'r') :
        if a.split("\t")[0] == gene and a.split("\t")[9].strip("\n") != "FPKM" :
            val[1] += (float(a.split("\t")[9].strip("\n"))+1)
            break
    for a in open(sys.argv[2],'r') :
        if a.split("\t")[0] == gene and a.split("\t")[9].strip("\n") != "FPKM" :
            val[2] += (float(a.split("\t")[9].strip("\n"))+1)
            break
    for a in open(sys.argv[3],'r') :
        if a.split("\t")[0] == gene and a.split("\t")[9].strip("\n") != "FPKM" :
            val[3] += (float(a.split("\t")[9].strip("\n"))+1)
            break
    for a in open(sys.argv[4],'r') :
        if a.split("\t")[0] == gene and a.split("\t")[9].strip("\n") != "FPKM" :
            val[4] += (float(a.split("\t")[9].strip("\n"))+1)
            break
    for a in open(sys.argv[5],'r') :
        if a.split("\t")[0] == gene and a.split("\t")[9].strip("\n") != "FPKM" :
            val[5] += (float(a.split("\t")[9].strip("\n"))+1)
            break
    for a in open(sys.argv[6],'r') :
        if a.split("\t")[0] == gene and a.split("\t")[9].strip("\n") != "FPKM" :
            val[6] += (float(a.split("\t")[9].strip("\n"))+1)
            break
    if val != [gene,0,0,0,0,0,0] :
    #if 0 is not in val :
    #if any(t <= 1 for t in val) and (t >= 10 for t in val):
    #    continue
    #else :
