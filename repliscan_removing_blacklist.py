...
Methods to handle replicates:
  ...
  - max''')
	...
	parser.add_argument("--blacklist", action="store_true", help=\
                "Remove regions of 0 read coverage on G1 data")
	...
	#####################################################
	L, normVals = makeBedgraph(fList, args.ref, args.window, args.aggregate.lower(), 'Coverage', args.remove, args.plot, pLow, pHigh, args.blacklist)
	#####################################################
...
def makeBedgraph(fList, fasta, size, aggMethod, normMethod, removeWhat, plotCoverage, pLow, pHigh, blacklist):
	...
	if blacklist :
                print "BLACKLIST IS ON, generating G1_norm_blacklist.bedgraph"
		outfile = open(times[0]+'_norm_blacklist.bedgraph', 'w')
                for a in open(times[0]+'_norm.bedgraph', 'r') :
...
def makeGFF(fList, chromDict, level, S, plotCov, threshMethod, scope, useLog, thresh, pCut, segMeth, blacklist):
	fSuff = {True:'logFC', False:'ratio'}[useLog]
	### edit blacklist
        beds = map(lambda y: "%s_%s_%i.smooth.bedgraph"%(y[1], fSuff, level), fList[1:])
	if blacklist : 
		for bed in beds :
			os.system("bedtools subtract -a "+bed+" -b G1_norm_blacklist.bedgraph > "+bed.strip(".bedgraph")+"_sub.blacklist.bedgraph")
	...
	for chrom in sortedChroms:
		...
		for i in xrange(len(beds)):
			...
			OF.close()
                        if blacklist :
				os.system("bedtools subtract -a "+outGFF+" -b G1_norm_blacklist.bedgraph > "+outGFF.strip(".gff3")+"_sub.blacklist.gff3")
				## rename genes
				outfile = open(outGFF.strip(".gff3")+"_sub.blacklist.temp", 'w')
				for a in open(outGFF.strip(".gff3")+"_sub.blacklist.gff3", 'r') :
					outfile.write("\t".join(a.split("\t")[:-1])+"\t")
					outfile.write("ID=gene"+str(start)+";")
					outfile.write(a.split("\t")[-1].split(";")[1]+"\n")
					start += 1
				outfile.close()
				os.system("mv "+outGFF.strip(".gff3")+"_sub.blacklist.temp "+outGFF.strip(".gff3")+"_sub.blacklist.gff3") 
		## Segmentation
