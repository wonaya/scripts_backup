...
def main():
	...
	#-----------------------------------------------------
	#print config['methratio']
        #runRatio(config, outPrefix)
...
def runBSMAP(config, outPrefix, r2):
	...
	#print bsmapCMD ; sys.exit()
        #bsP = sp.Popen(bsmapCMD, stderr=fileerr, stdout=fileout)
        #bsP.stdout.close()
	#bsOUT = bsP.stderr.read()
        cpus = str(multiprocessing.cpu_count())
        
        #os.system('samtools view -bS %s.sam | samtools sort -@ %s - -o %s.bam'%(outPrefix, cpus, outPrefix))
        #sys.exit()
        if r2:
